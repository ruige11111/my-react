...
function pushEffect(
	...
	const updateOueue = fiber.updateQueue as FCUpdateQueue<any>;
	if (updateOueue === null) {
		const updateOueue = createFCUpdateQueue();
		fiber.updateQueue = updateOueue;
		effect.next = effect;
		updateOueue.lastEffect = effect;
	} else {
		// 插入effect
		const lastEffect = updateOueue.lastEffect;
		if (lastEffect === null) {
			effect.next = effect;
			updateOueue.lastEffect = effect;
		} else {
...
function createFCUpdateQueue<State>() {
	const updateOueue = createUpdateQueue<State>() as FCUpdateQueue<State>;
	updateOueue.lastEffect = null;
	return updateOueue;
}
...
function updateState<State>(): [State, Dispatch<State>] {
	...
	// 计算新state的逻辑
	const queue = hook.updateOueue as FCUpdateQueue<State>;
	...
	if (baseQueue !== null) {
		...
		const {
			...
			const skipedLane = update.lane;
			const fiber = currentlyRenderingFiber as FiberNode;
			// NoLanes
			fiber.lanes = mergeLanes(fiber.lanes, skipedLane);
		});
...
function updateWorkInProgressHook(): Hook {
	...
	const newHook: Hook = {
		...
		updateOueue: currentHook.updateOueue,
		next: null,
...
function dispatchSetState<State>(
	fiber: FiberNode,
	updateOueue: FCUpdateQueue<State>,
	action: Action<State>
) {
	...
	if (
		...
		// 1、更新前的状态 2、计算状态的方法
		const currentState = updateOueue.lastRenderedState;
		const eagerState = basicStateReducer(currentState, action);
		update.hasEagerState = true;
		update.eagerState = eagerState;

		if (Object.is(currentState, eagerState)) {
			enqueueUpdate(updateOueue, update, fiber, NoLane);
			// 命中eagerState
			if (__DEV__) {
	...
	enqueueUpdate(updateOueue, update, fiber, lane);
